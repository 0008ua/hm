...
export class ProductFormComponent implements OnInit, AfterViewInit {
        ...
        fabric.Image.fromURL(src,
            (oImg: fabric.Image) => {
              if (this.canvasPristine) {
                ...
                if (imgRatio >= this.initialCanvasSize.ratio) {
                // portrait v1
                ...
                } else if (imgRatio < this.initialCanvasSize.ratio && imgRatio >= 1) {
                // portrait v2
  ...
  uploadPicture() {
    ...
    // prepare url for preview in <img> while piture uploading
    // scale to original size, crop selected area, convert to url
    this.imgUrl = this.image.set({
      scaleX: 1,
      scaleY: 1,
    }).toDataURL(cropOptions);

    // create temp canvas from cropped area
    ...
    tmpCanvas.toBlob((blob) => {
      ...
      if (error) {
        ...
        this.sharedService.uploadPicture(file, 'product', [
          ...
          { width: 300, height: 450, crop: 'fill', fetch_format: 'auto' }, // md
        ])
            .subscribe((public_id) => {
